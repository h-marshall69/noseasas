...
export class PhotoService {
    ...
    static async getPhotosDni(bodyRequest) {
        const controller = new AbortController()

        try {
            const response = await apiClient.post('/api/fotos_dni', bodyRequest, {
                // const response = await apiClient.get(`/api/fotos_dni/${bodyRequest}`, {
                signal: controller.signal,
                headers: { 'Content-Type': 'multipart/form-data' },
            })
            return {
